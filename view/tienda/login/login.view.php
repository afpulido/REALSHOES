
<!-- Start login -->
<section id="contact-us" class="contact-us section">
	<div class="container">
				<div class="contact-head">
					<div class="row">
						<div class="col-lg-3 col-12">
							
						</div>
						<div class="col-lg-6 col-12">
							<div class="form-main">
								<div class="title">
									<h1 style="color: #fd7e14;">Iniciar sesión</h1>
									<br>
									<h3>Digite sus credenciales</h3>
								</div>
								<form id="formulario" class="form" method="post" action="#">
									<div class="row">
										<div class="col-lg-8 col-12">
											<div class="form-group">
												<label >Correo <span>  *</span></label>
													<input name="email" type="email" placeholder="Ingrese su Correo" required>
													<span class="error"></span>
												</label>
												
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-lg-8 col-12">
											<div class="form-group">
												<label>Contraseña<span>*</span></label>
												<input name="password" type="password" placeholder="Ingrese su contraseña" required>
												<span class="error"></span>
											</div>
										</div>
									</div>
									<div class="row">
										<div class="col-lg-5 col-12">
											<div class="form-group">
												<a href="?c=Tiendas&a=olvido">¿Olvido su contraseña?</a>
											</div>
										</div>
									
										<div class="col-lg-1 col-12">
											<div class="form-group button">
												<button type="submit" class="btn "  id='btn-login' onclick="botonMensaje(1)">Ingresar</button>
											</div>
										</div>
									</div>
								</form>
							</div>
						</div>
						<div class="col-lg-3 col-12">
							
						</div>
					</div>
				</div>
			</div>
</section>
	<!--/ End login -->


	

